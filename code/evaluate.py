...
def evaluate(testset):
    ...
    page_names = [
        "data/{}/page.{}".format(testset, page_num)
        for page_num in range(1, NUM_TEST_PAGES + 1)
    ]

    # Load the correct labels for each test page
    true_labels = [utils.load_labels(page_name) for page_name in page_names]

    # Load the 10-dimensional feature data for each test page
    page_data_all_pages = [
        system.load_test_page(page_name, model) for page_name in page_names
    ]
    ...
    output_labels = [
        system.classify_page(page_data, model) for page_data in page_data_all_pages
    ]

    if "correct_errors" in dir(system):
        bboxes = [load_bounding_box(page_name) for page_name in page_names]
        output_labels = [
            system.correct_errors(p, o, b, model)
            for p, o, b in zip(page_data_all_pages, output_labels, bboxes)
        ]

    # Compute the percentage correct classifications for each test page
    scores = [
        (100.0 * np.sum(output_label == true_label)) / output_label.shape[0]
...
def usage():
    ...
    print("Usage: python evaluate.py <testset>")
